...
export async function fetchSystemHealth(
  ...
  const endpoint =
    baseUrl ?? `${import.meta.env.VITE_API_URL ?? "/api"}/health`;
...
export async function fetchMetadataFromBackend(
  baseUrl?: string,
): Promise<Document[]> {
  const endpoint =
    baseUrl ?? `${import.meta.env.VITE_API_URL ?? "/api"}/metadata`;
...
export async function updateDocumentInBackend(
  ...
  >,
  baseUrl?: string,
): Promise<{ ok: boolean; metadata?: Document["metadata"] }> {
  const endpoint =
    baseUrl ?? `${import.meta.env.VITE_API_URL ?? "/api"}/documents`;
