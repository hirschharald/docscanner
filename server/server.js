...
const connectionString =
  ...
  "postgresql://docscanner:docscanner@localhost:5433/docscanner";
...
function extractYear(document) {
  const metadataYear =
    document?.metadata?.Jahr ||
    document?.metadata?.jahr ||
    document?.metadata?.year;
  const parsedMetadataYear = Number.parseInt(String(metadataYear || ""), 10);

  if (
    Number.isInteger(parsedMetadataYear) &&
    parsedMetadataYear >= 1900 &&
    parsedMetadataYear <= 2100
  ) {
...
async function persistDocumentMetadata(document, fileInfo) {
  const metadataEntries = Array.isArray(document?.tags)
    ? parseMetadataTags(document.tags)
    : {};
  ...
  if (updatedEntry) {
    ...
    if (oldFilePath && newFilePath && oldFilePath !== newFilePath) {
      try {
        ...
        return res
          .status(500)
          .json({ ok: false, message: "Could not move document file" });
      }
    }
  }
...
app.post("/api/documents", async (req, res) => {
  ...
  documents.forEach((document, index) => {
    ...
    console.log(
      `[${index + 1}] ${document?.name || "unnamed"} | tags: ${tags} | metadata:`,
      document?.metadata || {},
    );
  });
  ...
  res.json({
    ...
    message:
      failed.length === 0
        ? "Documents received and stored successfully"
        : "Documents received with partial storage failures",
  });
});

app.get("/api/health", async (_req, res) => {
  try {
    await metadataStore.ping();
    return res.json({
      ok: true,
      backend: true,
      db: true,
  ...
  if (filePath) {
    try {
      ...
      if (errorCode !== "ENOENT") {
        return res
          .status(500)
          .json({ ok: false, message: "Could not delete document file" });
      }
    }
  }
